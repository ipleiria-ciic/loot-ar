...
public class ImgObfuscator : MonoBehaviour
{
    ...
    {
        // Ensure the input texture is readable by converting it to a Texture2D if it's not already
        RenderTexture temporaryRT = RenderTexture.GetTemporary(inpTexture.width, inpTexture.height, 0, RenderTextureFormat.Default, RenderTextureReadWrite.Linear);

        Graphics.Blit(inpTexture, temporaryRT);
        RenderTexture previous = RenderTexture.active;
        RenderTexture.active = temporaryRT;

        Texture2D outTexture = new Texture2D(inpTexture.width, inpTexture.height, TextureFormat.RGBA32, false);
        outTexture.ReadPixels(new Rect(0, 0, temporaryRT.width, temporaryRT.height), 0, 0);
        outTexture.Apply();
    ...
    {
        // Ensure the mask tensor is readable
        maskTensor.MakeReadable();
        ...
        // Ensure the binarized masks tensor is readable
    ...
    )
    {
        ...
        switch (obfuscationType)
        {
            case Obfuscation.Type.Masking:
                ImgUtils.MaskTexture(_outTexture, boolCrpMsk, Color.black);
    ...
    private static TensorFloat ResizeTF_v3(TensorFloat input, float scaleX, float scaleY, bool invertY = false)
    {
        // Ensure the input tensor data is readable
        input.MakeReadable();
        ...
        {
            // Schedule the job
            JobHandle handle = job.Schedule(arrayLength: outputWidth * outputHeight, innerloopBatchCount: 64);

            // Wait for the job to complete
            handle.Complete();

            // Convert the output NativeArray to a TensorFloat
